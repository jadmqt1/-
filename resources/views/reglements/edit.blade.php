                ...
                <div class="w-full max-w-full px-3 mx-auto mt-0 md:flex-0 shrink-0 md:w-7/12 lg:w-5/12 xl:w-4/12">
                    <div
                        ...
                        <div class="flex-auto p-6">
                            <form method="post" action="{{ route('reglements.update', $reglements->id) }}">
                                @csrf
                                @method('put')

                                <div class="mb-4">
                                    <label for="date">Date</label>
                                    <input name="date" value="{{ $reglements->date }}" type="text"
                                ...
                                <div class="mb-4">
                                    ...
                                    <input name="montant" value="{{ $reglements->montant }}" type="text"
                                        class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-control text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                                        ...
                                        @foreach($livraisons as $livraison)
                                            <option value="{{ $livraison->id }}" {{ $livraison->id == $reglements->bon_livraison_id ? 'selected' : '' }}>
                                                {{ $livraison->id }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    <label for="mode_regelment_id">Mode Règlement</label>
                                    <select class="form-control text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                                            name="mode_regelment_id">
                                        @foreach($modes as $mode)
                                            <option value="{{ $mode->id }}" {{ $mode->id == $reglements->mode_regelment_id ? 'selected' : '' }}>
                                                {{ $mode->mode }}
                                            </option>
                                        @endforeach
                                    </select>
